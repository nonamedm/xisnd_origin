...
(function()
{
	function addCombo( editor, comboName, styleType, lang, entries, defaultLabel, styleDefinition )
	{
		...
		editor.ui.addRichCombo( 
				...
				
				{
					...
					label : lang.line_height.label,
					title: lang.line_height.panelTitle,
					className: 'cke_' + (styleType == 'size' ? 'fontSize' : 'font'),
					panel :{css : [CKEDITOR.skin.getPath("editor")].concat( config.contentsCss ),
					multiSelect : false,
					attributes: { 'aria-label': lang.line_height.panelTitle }
					
				},  
	  
				init : function(){
					
					this.startGroup( lang.line_height.panelTitle );  
	...
	CKEDITOR.plugins.add('line_height',
	{
	    lang: ['ko-kr'],
		requires : [ 'richcombo' ],
		...
		init : function( editor )
		{
			...
			addCombo(editor, 'line_height', 'size', editor.lang, config.lineheight_sizes, config.fontSize_defaultLabel, config.lineheight_style);
		}
	});
})();
